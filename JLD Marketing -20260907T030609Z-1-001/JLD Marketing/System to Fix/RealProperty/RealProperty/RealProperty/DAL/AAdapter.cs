...
internal abstract class AAdapter
{
	...
	internal int addBulk(dynamic listEntity)
	{
		...
		try
		{
			mySqlCommand.CommandTimeout = DatabaseConnection.CommandTimeoutSeconds;
			...
			{
				DatabaseConnection.Log(exception, "AAdapter.addBulk");
	...
	private int executeNonQuery(MySqlCommand cmd)
	{
		...
		try
		{
			...
			{
				DatabaseConnection.Log(exception, "AAdapter.executeNonQuery");
				if (mySqlTransaction != null)
				{
					mySqlTransaction.Rollback();
				}
	...
	private object ExecuteScalar(MySqlCommand cmd)
	{
		...
		try
		{
			...
			{
				DatabaseConnection.Log(exception, "AAdapter.ExecuteScalar");
				if (mySqlTransaction != null)
				{
					mySqlTransaction.Rollback();
				}
	...
	private object ExecuteInsertQuery(MySqlCommand cmd)
	{
		...
		try
		{
			...
			{
				DatabaseConnection.Log(exception, "AAdapter.ExecuteInsertQuery");
				if (mySqlTransaction != null)
				{
					mySqlTransaction.Rollback();
				}
	...
	private DataTable getTable(MySqlCommand cmd)
	{
		...
		{
			DatabaseConnection.Log(exception, "AAdapter.getTable");
			return null;
		}
	}
	...
	private DataTable getTable<T>(MySqlCommand cmd)
	{
		...
		{
			DatabaseConnection.Log(exception, "AAdapter.getTable<T>");
			return null;
		}
	}
