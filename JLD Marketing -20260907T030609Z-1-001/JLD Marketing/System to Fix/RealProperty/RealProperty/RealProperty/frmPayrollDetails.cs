...
public class frmPayrollDetails : Form
{
	...
	private DataGridViewTextBoxColumn idpayrolldeduc_col;

	private DataGridViewTextBoxColumn title_col;

	private DataGridViewTextBoxColumn totalamount_col;

	private DataGridViewTextBoxColumn totalpaid_col;

	private DataGridViewTextBoxColumn amountpaid_col;

	private DataGridViewTextBoxColumn balance_col;

	private DataGridViewButtonColumn edit_amount_col;
	...
	{
		refresh_DGVEmployeeSalary();
		DGVEmployeeSalary_CellClick(null, null);
	}

	private void DGVEmployeeSalary_CellClick(object sender, DataGridViewCellEventArgs e)
	{
		if (DGVEmployeeSalary.CurrentRow == null)
		{
			return;
		}
		if (DGVEmployeeSalary.CurrentRow.Index >= 0)
		{
			using (PayrollCtrl payrollCtrl = new PayrollCtrl())
			{
				...
				sel_payrolldetails = payrollCtrl.getPayrollTableByEmployeeID(sel_payrolldetails);
			}
		...
		Refresh_DGVDeduc(id);
		if (DGVdeduc.Rows.Count > 0)
		{
			MessageBox.Show("Please remove all deduction first!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
			return;
		}
	...
	private void btnAddDeduction_Click(object sender, EventArgs e)
	{
		...
		using frmSearchLoans frmSearchLoans2 = new frmSearchLoans();
		frmSearchLoans2.idemployee = (object)value;
		frmSearchLoans2.employeename = value2;
		frmSearchLoans2.searchType = "PAYROLL";
		frmSearchLoans2.ShowDialog(this);
		if (frmSearchLoans2.selected_idloan <= 0 || !(frmSearchLoans2.selected_balance > 0m))
		{
			return;
		}
		using PayrollCtrl payrollCtrl = new PayrollCtrl();
		int num = payrollCtrl.add_payrolldeduction(payroll.id, frmSearchLoans2.selected_balance, frmSearchLoans2.selected_idloan);
		if (num > 0)
		{
			Refresh_DGVDeduc(value);
		}
		else
		{
			MessageBox.Show("Record not saved!", "Message", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
		}
	}
	...
	{
		using PayrollCtrl payrollCtrl = new PayrollCtrl();
		DataTable dataTable = payrollCtrl.get_view_payrolldeduction_withbalance(id, payroll.id);
		string filterExpression = string.Format("category not like '%{0}%' AND category not like '%{1}%' AND recordstatus='active'", "BENEFIT", "EARNING");
	...
	private void num_Value(object sender, EventArgs e)
	{
		NumericUpDown numericUpDown = sender as NumericUpDown;
		if (numericUpDown.Value > 0m)
		{
			numericUpDown.ForeColor = SystemColors.ControlText;
			numericUpDown.Parent.BackColor = SystemColors.Control;
		}
		else
		{
			numericUpDown.ForeColor = SystemColors.GrayText;
			numericUpDown.Parent.BackColor = Color.Red;
		}
	}

	private void DGVdeduc_CellClick(object sender, DataGridViewCellEventArgs e)
	{
		if (e == null)
		{
			return;
		...
		int columnIndex = e.ColumnIndex;
		int id = Convert.ToInt32(DGVEmployeeSalary.CurrentRow.Cells[idemployeeDataGridViewTextBoxColumn.Name].Value);
		int idpayrolldeduc = Convert.ToInt32(DGVdeduc.CurrentRow.Cells[idpayrolldeduc_col.Name].Value);
		if (columnIndex == delete_deduction_col.Index)
		{
			DialogResult dialogResult = MessageBox.Show("Are you sure you want to delete this record?", "Information", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk);
			if (dialogResult != DialogResult.Yes)
			{
				return;
			}
			using PayrollCtrl deletePayrollCtrl = new PayrollCtrl();
			Payrolldeduction payrollDeducObject = deletePayrollCtrl.getPayrollDeducObject(idpayrolldeduc);
			payrollDeducObject.recordstatus = "deleted";
			int num = deletePayrollCtrl.edit_payrolldeduction(payrollDeducObject);
			if (num > 0)
			{
				Refresh_DGVDeduc(id);
			}
		...
		if (columnIndex != edit_amount_col.Index)
		{
			return;
		}
		using frmPayrolldeductionEditAmount frmPayrolldeductionEditAmount2 = new frmPayrolldeductionEditAmount();
		using PayrollCtrl editPayrollCtrl = new PayrollCtrl();
		Payrolldeduction payrolldeduction = (frmPayrolldeductionEditAmount2.payrolldeduction = editPayrollCtrl.getPayrollDeducObject(idpayrolldeduc));
		if (payrolldeduction != null)
		{
			frmPayrolldeductionEditAmount2.ShowDialog(this);
			if (frmPayrolldeductionEditAmount2.commitchanges)
			{
				Refresh_DGVDeduc(id);
			}
		}
	...
	private void InitializeComponent()
	{
		...
		this.idpayrolldeduc_col = new System.Windows.Forms.DataGridViewTextBoxColumn();
		...
		this.totalpaid_col = new System.Windows.Forms.DataGridViewTextBoxColumn();
		this.amountpaid_col = new System.Windows.Forms.DataGridViewTextBoxColumn();
		this.balance_col = new System.Windows.Forms.DataGridViewTextBoxColumn();
		this.edit_amount_col = new System.Windows.Forms.DataGridViewButtonColumn();
		...
		this.numMon.ValueChanged += new System.EventHandler(num_Value);
		...
		this.numTue.ValueChanged += new System.EventHandler(num_Value);
		...
		this.numWed.ValueChanged += new System.EventHandler(num_Value);
		...
		this.numThu.ValueChanged += new System.EventHandler(num_Value);
		...
		this.numFri.ValueChanged += new System.EventHandler(num_Value);
		...
		this.numSat.ValueChanged += new System.EventHandler(num_Value);
		...
		this.DGVdeduc.Columns.AddRange(this.idpayrolldeduc_col, this.title_col, this.totalamount_col, this.totalpaid_col, this.amountpaid_col, this.balance_col, this.edit_amount_col, this.delete_deduction_col);
		...
		this.DGVdeduc.CellClick += new System.Windows.Forms.DataGridViewCellEventHandler(DGVdeduc_CellClick);
		this.panel2.Dock = System.Windows.Forms.DockStyle.Bottom;
		this.panel2.Location = new System.Drawing.Point(0, 537);
		this.panel2.Name = "panel2";
		this.panel2.Size = new System.Drawing.Size(754, 54);
		this.panel2.TabIndex = 17;
		this.idpayrolldeduc_col.DataPropertyName = "id";
		this.idpayrolldeduc_col.HeaderText = "id";
		this.idpayrolldeduc_col.Name = "idpayrolldeduc_col";
		this.idpayrolldeduc_col.ReadOnly = true;
		this.idpayrolldeduc_col.Visible = false;
		...
		this.totalamount_col.DataPropertyName = "totalamount";
		this.totalamount_col.HeaderText = "totalamount";
		this.totalamount_col.Name = "totalamount_col";
		this.totalamount_col.ReadOnly = true;
		this.totalpaid_col.DataPropertyName = "totalpaid";
		this.totalpaid_col.HeaderText = "totalpaid";
		this.totalpaid_col.Name = "totalpaid_col";
		this.totalpaid_col.ReadOnly = true;
		this.amountpaid_col.DataPropertyName = "amount";
		this.amountpaid_col.HeaderText = "amount paid";
		this.amountpaid_col.Name = "amountpaid_col";
		this.amountpaid_col.ReadOnly = true;
		...
		this.edit_amount_col.AutoSizeMode = System.Windows.Forms.DataGridViewAutoSizeColumnMode.AllCellsExceptHeader;
		dataGridViewCellStyle2.Alignment = System.Windows.Forms.DataGridViewContentAlignment.MiddleCenter;
		dataGridViewCellStyle2.ForeColor = System.Drawing.Color.Blue;
		this.edit_amount_col.DefaultCellStyle = dataGridViewCellStyle2;
		this.edit_amount_col.FlatStyle = System.Windows.Forms.FlatStyle.Flat;
		this.edit_amount_col.HeaderText = "edit";
		this.edit_amount_col.Name = "edit_amount_col";
		this.edit_amount_col.ReadOnly = true;
		this.edit_amount_col.Resizable = System.Windows.Forms.DataGridViewTriState.True;
		this.edit_amount_col.SortMode = System.Windows.Forms.DataGridViewColumnSortMode.Automatic;
		this.edit_amount_col.Text = "edit";
		this.edit_amount_col.UseColumnTextForButtonValue = true;
		this.edit_amount_col.Width = 5;
