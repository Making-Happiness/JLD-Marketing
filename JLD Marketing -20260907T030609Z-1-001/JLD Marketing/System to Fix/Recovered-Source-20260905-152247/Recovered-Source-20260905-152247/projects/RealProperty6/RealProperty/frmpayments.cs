...
public class frmpayments : Form
{
	...
	private void btnAcceptPayment_Click(object sender, EventArgs e)
	{
		...
		if (num > 0)
		{
			DialogResult dialogResult = MessageBox.Show("Payments Successfull\nDo you wnat to continue payment?", "Information", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk);
	...
	{
		cboFullname.DataBindings.Clear();
		cboPaymentFor.DataBindings.Clear();
		numAmount.DataBindings.Clear();
		dtpDateofPayment.DataBindings.Clear();
		txtOR.DataBindings.Clear();
		txtReferenceNo.DataBindings.Clear();
		cboRecieveByIncharge.DataBindings.Clear();
		cboPaymentType.DataBindings.Clear();
		txtPaymentNo.DataBindings.Clear();
		cboPaidBy.DataBindings.Clear();
	}
