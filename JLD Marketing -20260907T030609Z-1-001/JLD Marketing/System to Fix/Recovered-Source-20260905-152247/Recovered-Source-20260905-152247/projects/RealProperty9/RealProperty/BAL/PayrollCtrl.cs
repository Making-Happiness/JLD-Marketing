...
internal class PayrollCtrl : AController, IDisposable
{
	...
	internal Payrolldetails getPayrollTableByEmployeeID(int idemployee)
	{
		DataTable tableByID = Adapter.getTableByID("payrolldetails", "idemployee", idemployee);
		if (tableByID.Rows.Count > 0)
		{
			return AController.DataTableToList<Payrolldetails>(tableByID).First();
		}
		return null;
	}
