...
public class frmApplicationForm : Form
{
	...
	private void bindDataPurchasedetails()
	{
		...
		numMonthlypayment.DataBindings.Add("Value", purchasedetail, "amortization", formattingEnabled: false, DataSourceUpdateMode.OnPropertyChanged);
		...
		purchasedetail.duedate = ((purchasedetail.duedate == DateTime.MinValue) ? DateTime.Today.AddYears(int.Parse("0" + cboTerms.Text)) : purchasedetail.duedate);
	...
	private void btnSave_Click(object sender, EventArgs e)
	{
		...
		purchasedetail.duedate = ((purchasedetail.duedate <= dtpDueDate.MinDate) ? DateTime.MinValue : purchasedetail.duedate);
		int num;
	...
	private void cboTerms_SelectedIndexChanged(object sender, EventArgs e)
	{
		if (purchasedetail.id == 0)
		{
			dtpDueDate.Value = DateTime.Today.AddYears(int.Parse("0" + cboTerms.Text));
