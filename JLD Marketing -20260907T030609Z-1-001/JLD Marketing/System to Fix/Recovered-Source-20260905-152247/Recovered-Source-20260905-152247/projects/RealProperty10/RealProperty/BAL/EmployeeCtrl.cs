...
internal class EmployeeCtrl : AController, IDisposable
{
	...
	internal int generatePayrollDetailsSingle(int idpayroll, int idemployee)
	{
		Dictionary<string, object> dictionary = new Dictionary<string, object>();
		dictionary.Clear();
		dictionary.Add("vidpayroll", idpayroll);
		dictionary.Add("videmployee", idemployee);
		return Adapter.executeProcedure("proc_generate_payrolldetails_single", dictionary);
	}

	internal int generate_payslipdetailsSingle(int idpayslip, int idemployee)
	{
		Dictionary<string, object> dictionary = new Dictionary<string, object>();
		dictionary.Add("vidpayslip", idpayslip);
		dictionary.Add("videmployee", idemployee);
		return Adapter.executeProcedure("proc_generate_payslipdetails_single", dictionary);
	}
}
