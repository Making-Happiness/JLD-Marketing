...
public class frmPayrollDetails : Form
{
	...
	private void DGVdeduc_CellClick(object sender, DataGridViewCellEventArgs e)
	{
		...
		if (columnIndex == delete_deduction_col.Index)
		{
			...
			using PayrollCtrl payrollCtrl = new PayrollCtrl();
			Payrolldeduction payrollDeducObject = payrollCtrl.getPayrollDeducObject(idpayrolldeduc);
			payrollDeducObject.recordstatus = "deleted";
			int num = payrollCtrl.edit_payrolldeduction(payrollDeducObject);
			if (num > 0)
			{
				Refresh_DGVDeduc(id);
			}
		...
		using PayrollCtrl payrollCtrl = new PayrollCtrl();
		Payrolldeduction payrolldeduction = (frmPayrolldeductionEditAmount2.payrolldeduction = payrollCtrl.getPayrollDeducObject(idpayrolldeduc));
