...
internal class PayrollCtrl : AController, IDisposable
{
	...
	internal DataTable get_view_payrolldeduction_withbalance(int idemployee, int idpayroll)
	{
		Dictionary<string, object> dictionary = new Dictionary<string, object>();
		dictionary.Add("idpayroll", idpayroll);
		dictionary.Add("idemployee", idemployee);
		return Adapter.getTable("view_payrolldeduction_withbalance", dictionary, 0, 0, "WHERE idpayroll=?idpayroll AND idemployee=?idemployee");
	}
